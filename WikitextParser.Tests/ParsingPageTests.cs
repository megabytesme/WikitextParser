...
public class ParsingPageTests
{
    ...
    public void ParsePage_FullFictionalArticle_BuildsCorrectAndCompleteHierarchy()
    {
        ...
        // Check that there are 3 paragraphs and the first one has the correct starting text.
        var leadParagraphs = page.LeadContent.OfType<ParagraphElement>().ToList();
        leadParagraphs.Should().HaveCount(3);
        leadParagraphs.First().ConvertToText().Should().StartWith("Eerie Hollow is a fictional American television series");
        ...
        productionSection.Subsections[0].ContentElements.Should().HaveCount(2);
        ...
        table.Rows.Should().HaveCount(3); // 1 header and 2 data rows
